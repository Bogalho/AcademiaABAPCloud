CLASS zcl_fabap_jul2025_exer04_rpb DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    TYPES t_conn TYPE STANDARD TABLE OF zi_fabap_demo01_eat WITH EMPTY KEY.

    CLASS-METHODS get_data AMDP OPTIONS CDS SESSION CLIENT DEPENDENT
      EXPORTING VALUE(et_data) TYPE t_conn.
...
CLASS ZCL_FABAP_JUL2025_EXER04_RPB IMPLEMENTATION.


  METHOD get_data BY DATABASE PROCEDURE FOR HDB LANGUAGE SQLSCRIPT
  OPTIONS READ-ONLY USING zi_fabap_demo01_eat.
    et_data = select Connection.carrierid     as CarrierId,
                     ...
                     case
                     when Connection.airportfromid = 'SFO'then '080000'
